function Imagenes
    ...
    % tiempo (regular) entre estímulos, en segundos
    INTERVAL = 0.5;
    ...
    if TARGET_IS_IMAGE && ~IMG
        fprintf('\nERROR: IMG vale 0 (no usar imagenes) y el target es IMG (TARGET_IS_IMAGE == 1). Así no tendría mucho sentido.\n')
        quit
    end
    if ~TARGET_IS_IMAGE && ~SND
        fprintf('\nERROR: SND vale 0 (no usar imagenes) y el target es SND (TARGET_IS_IMAGE == 0). Así no tendría mucho sentido.\n')
        quit
    end
    if (IMG ~= SND) && (DELAY ~= 0)
        DELAY = 0
    ...
    try
        ...
        tapKeyCode = 66; % barra espaciadora
        time_samples = [];
...
% Capaz conviene normalizar esto con el intervalo entre estímulos?
